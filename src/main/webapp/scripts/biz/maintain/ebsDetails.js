...
//		state : '',
		...
		getRelateHosts : function(){
			...
			Dcp.biz.apiRequest("/instance/ebs/describeIris", params, function(data) {
				if (data.code != "0") {
					$.growlUI(Dict.val('key_instance_hint','提示'), Dict.val('key_js_searchResource_buy_runing','查询用户已经购买的运行状态的关联资源发生错误')+"：" + data.msg); 
				} else {
					var array = data.data;
		...
		initEbsById : function(ebsId){
			Dcp.biz.apiRequest("/instance/ebs/describeEbsDetail", {"id":ebsId}, function(data) {
				if (data.code != "0") {
					$.growlUI(Dict.val('key_instance_hint','提示'), Dict.val('key_js_searchResource_detail_error','查询资源的详细信息发生错误')+"：" + data.msg); 
				} else {
	...
	detachDataTable : function(data) {
		ebsDetail.hostDatatable.renderByData("#ebsUnloadTable", {
			...
			"columnDefs" : [
			     ...
			     {title : Dict.val('key_instance_name','名称'), name : "vmInstanceInfoName"}
			],
	...
	detachEbs : function(id) {
		...
		var _hostIds = volumeIds.join(",");
				...
				Dcp.biz.apiRequest("/instance/ebs/detachEbsVolumes", params, function(data){
					if(data.code == "0"){
						$.growlUI(Dict.val('key_instance_hint','提示'), Dict.val('key_js_unmountEbs_success','解挂弹性块存储成功')+"！"); 
						$("#ebsUnload").modal("hide");
					} else {
						$.growlUI(Dict.val('key_instance_hint','提示'), Dict.val('key_js_unmountEbs_error','解挂弹性块存储发生错误')+"：" + data.msg); 
					}
				});
	}
...
function calc() {
	...
	$(function(){
		...
		if(index==0){
			...
			if($("#state").text() == Dict.val('key_js_unmount','未挂载')){
				$("#modifyEbsStorageSizeModal").modal("show");
			}else{
				$.growlUI(Dict.val('key_instance_hint','提示'), Dict.val('key_js_capacityAdd_again','扩容只能对未挂载的弹性块存储进行操作,请重新选择')); 
			}
		}
		else if(index==2){
